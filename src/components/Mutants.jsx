...
const Mutants = ({ tableItems, term }) => {

  const visItems2 = tableItems.mutants.filter(item => {
    return item.label.toLowerCase().indexOf(term.toLowerCase()) > -1;
  })

  const elements = visItems2.map((item) => {
...
export default connect(mapStateToProps)(Mutants);
